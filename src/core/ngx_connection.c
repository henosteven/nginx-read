...
//这里就是单纯的获取一个connection，然后是一些数据的设置
//具体的handler应该是要跟connection不相关，应该是在http_request或者是其他的逻辑处理如mail，设计也是如此
ngx_connection_t *
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
    ...
    //这里的设置把connection设置都到写事件的数据项里面
    //因为epoll遍历事件的时候，取出来的是事件的数据
    //因为事件的数据项就是connection，所以就关联出来了
    rev->data = c;
    wev->data = c;
