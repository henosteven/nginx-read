...
static ngx_command_t  ngx_core_commands[] = {
    ...
    { ngx_string("daemon"),
      ...
      0,
      offsetof(ngx_core_conf_t, daemon), //指明要保存值的位置
      NULL },
    ...
    { ngx_string("load_module"),
      ...
      //这种实现方式方便判断结尾，而不需要关心数组中item的个数
      ngx_null_command
};


//模块上下文，保存了一组操作，Nginx初始化阶段调用这些操作，初始化ngx_core_conf_t中的成员
...
ngx_module_t  ngx_core_module = {
    NGX_MODULE_V1,
    &ngx_core_module_ctx,                  /* module context 可以理解为就是一些conf配置信息*/
...
main(int argc, char *const *argv)
{
    ...
    //这个步骤中设置了conf_file 和 conf_prefix 和 prefix
    //conf_file = {len = 32, data = 0x6d2460 "/usr/local/nginx/conf/nginx.conf"}, conf_param = {len = 0, data = 0x0},
    //conf_prefix = {len = 22, data = 0x6d2460 "/usr/local/nginx/conf/nginx.conf"},  
    //prefix = {len = 17, data = 0x47c43a "/usr/local/nginx/"}
