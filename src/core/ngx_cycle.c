...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    //参考之前的old_cycle
    //例如nginx -s reload, 就需要考虑之前的nginx是什么情况，基本维持之前状态，避免申请内存导致系统出现问题
    ...
    //监听端口部分的逻辑在这里，因为nginx支持多个服务同时在线（可以设置多个端口，同一个端口也可以设置不同服务）
    ...
    //准备好存好上下文内存（初始化操作） ngx_max_module 为当前模块总数+128（NGX_MAX_DYNAMIC_MODULES）
    //struct ngx_cycle_s {
    //void                  ****conf_ctx;
    ...
    // core module 的处理，创建并初始化模块的 ngx_core_conf_t
    ...
    //这个结构很有意思包含了cycle和cycle->conf_ctx
    //从上面的逻辑中可以看出，cycle->conf_ctx保存在是所有core模块的ngx_core_conf_t
    //所以conf其实就是core层配合
