...
ngx_event_accept(ngx_event_t *ev)
{
    ...
    do {
        ...
        //接收到的链接，生成一个新的connection
        //所以得出设置事件处理的handler的逻辑在这之下，因为handler这时候才出场
        ...
        *log = ls->log;
        ...
        //用于记录当前的connection是从哪个listening产生的
        c->listening = ls;
        ...
        //初始化read 和 write 事件数据
        //read 和 write属于ngx_event_t，其中有处理事件的handler
        rev = c->read;
        wev = c->write;
        ...
        if (ngx_add_conn && (ngx_event_flags & NGX_USE_EPOLL_EVENT) == 0) {
            //如果使用epoll的话
            //这个步骤就是epoll_ctl的操作，添加到事件机制里面
            //对事件处理的handler应该设置在前面的逻辑中

            //但是现在看见的真实情况是在下面的ls->hanlder中处理了添加了处理事件handler
            //epoll在这里有特殊处理，其他模式添加回去事件池，在下面的handler中搞定
        ...
        //这里完成了对新链接的处理，但是这里的具体的handler是什么呢？
        //在哪里设置的？nginx有很多这样的处理方式，导致代码不太好一眼看明白，但是设计非常精巧
        // *** 在前期，初始化ls的时候，设置的ls的handler为ngx_http_init_connection
        ls->handler(c);
