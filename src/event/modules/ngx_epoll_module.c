...
//将ngx_connection_t添加到相应epoll的事件列表中
//作为模块化的实现，这里肯定不会去考虑设置handler之类的东西
static ngx_int_t
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    for (i = 0; i < events; i++) {
        ...
        //此时最希望就是能够把这个C打印出来，让我看看从而加进去的
        //因为c->read->handler
        //现在非常关心这里的handler
        //这里的C应该是ngx_connetion_t
        //这个C应该是add_conn加进去的
        ...
        if ((revents & EPOLLIN) && rev->active) {
            ...
            if (flags & NGX_POST_EVENTS) {
                ...
                //这里又出现了handler，在哪儿设置的这个handler呢？
                //会不会因为业务（http,mail,proxy）的不同，而不一样呢？
                rev->handler(rev);
            }
        }
